"""底层纯函数：DuckDB 持久化连接，统一使用 analysis.duckdb"""
import json
from typing import Callable, Optional
from ...workspace import Workspace


def _get_connection(ws: Workspace):
    import duckdb
    return duckdb.connect(ws.duckdb_path)


def _quote_ident(name: str) -> str:
    return '"' + name.replace('"', '""') + '"'


def duckdb_query_impl(
    ws: Workspace,
    sql: str,
    emit_log: Optional[Callable[[str, str | dict], None]] = None,
) -> str:
    con = _get_connection(ws)
    try:
        result = con.execute(sql).fetchdf()
        output = json.dumps(json.loads(result.to_json(orient="records")), ensure_ascii=False)
        if emit_log:
            emit_log("custom_agent", {"level": "info", "message": f"✅ duckdb_query 调用成功: {sql[:80].replace(chr(10), ' ')}"})
        return output
    finally:
        con.close()


def duckdb_register_parquet_impl(
    ws: Workspace,
    table_name: str,
    parquet_path: str,
    emit_log: Optional[Callable[[str, str | dict], None]] = None,
) -> str:
    from pathlib import Path
    con = _get_connection(ws)
    try:
        safe_table = _quote_ident(table_name)
        p = Path(parquet_path)
        if not p.is_absolute():
            p = ws.resolve_read(parquet_path)
        abs_path = str(p.resolve())
        con.execute(f"CREATE OR REPLACE VIEW {safe_table} AS SELECT * FROM read_parquet('{abs_path}')")
        row_count = con.execute(f"SELECT COUNT(*) FROM {safe_table}").fetchone()[0]
        result = f"表 {table_name} 已注册, {row_count} 行"
        if emit_log:
            emit_log("custom_agent", {"level": "info", "message": f"✅ duckdb_register_parquet 调用成功: {table_name} <- {parquet_path}"})
        return result
    finally:
        con.close()
