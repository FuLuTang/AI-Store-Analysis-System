...
| 参数 | 类型 | 必选 | 说明 |
| --- | --- | --- | --- |
| `X-FZT-Key` | string | 是 | 用户认证密钥 |

请求 Query 参数：

| 参数 | 类型 | 必选 | 说明 |
| --- | --- | --- | --- |
| `run_id` | string | 否 | 待获取的运行批次 ID。缺省时自动获取当前最新一次运行的状态与结果 |
...
- `pipeline`: 启动管线
- `system`: 系统或Agent节点日志，数据格式如 `{"node": "NodeName", "message": "Log content"}`
- `state`: 进度更新
...
| 参数 | 类型 | 必选 | 说明 |
| --- | --- | --- | --- |
| `x-fzt-key` | string | 是（或在 Header 传 X-FZT-Key） | 用户认证密钥 |
| `run_id` | string | 否 | 待下载的分析运行批次 ID。缺省时自动使用最新一次运行的产物 |

响应：

- `200 OK`：返回 `application/zip` 二进制压缩文件流。
- `404 Not Found`：未找到对应运行记录或运行目录不存在。
- `500 Internal Server Error`：生成或读取 ZIP 压缩包失败。
  ...
  "runs": [
    {
      "runId": "20260527T161829_f518e8",
      "taskType": "diagnosis",
      "createdAt": "2026-05-27 16:18:29",
      "status": "completed",
      "fileNames": ["概览-日.json"],
      "isPublic": true,
      "shareUrl": "http://localhost:3000/public.html?run_id=20260527T161829_f518e8&sign=8a7c2b..."
    }
  ]
}
```
...
```json
{
  "status": "ok",
  "isPublic": true,
  "shareUrl": "http://localhost:3000/public.html?run_id=xxx&sign=yyy"
}
```
...
| 参数 | 类型 | 必选 | 说明 |
| --- | --- | --- | --- |
| `run_id` | string | 是 | 分析运行批次 ID |
| `sign` | string | 是 | 安全防伪签名 |
...
> 地址: https://api.shangboshop.com/Other/OtherErpApi/index
> 文档: https://console-docs.apipost.cn/preview/6704c0602dc41c35/3b9f8d25b1ae985f?target_id=0b119a23-012a-42f7-a289-61cdbc630ba8
