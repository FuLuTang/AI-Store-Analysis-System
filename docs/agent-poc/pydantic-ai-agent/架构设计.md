...
Agent 管线新增以下 Python 依赖（已写入 `requirements.txt`）：

```bash
pip install pydantic-ai duckdb pyarrow
```

| 包 | 用途 |
|----|------|
| `pydantic-ai` | Pydantic AI Agent 框架，提供 Agent + tool registration + structured output |
| `duckdb` | 内存 SQL 引擎，注册 parquet 为表，执行 Agent 生成的 SQL |
| `pyarrow` | parquet 读写支持（pandas DataFrame → .parquet） |

部署脚本 `scripts/install.sh` 已通过 `pip install -r requirements.txt` 自动安装，无需手动操作。
