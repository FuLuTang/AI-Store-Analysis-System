...
flowchart TD
    ...
    subgraph 语义理解_AI辅助
        direction TB
        C --> D[Scene Classifier<br/>行业/业态/目标识别 AI]
        D --> E[Semantic Mapper<br/>字段语义映射 AI]
    end

    subgraph 标准数据与指标_纯算法
        direction TB
    ...
    subgraph AI报告生成
        direction LR
        J --> L[LLM Report Writer<br/>报告写作 AI]
        L --> M[Report Output<br/>完整报告 / 卡片 / API JSON AI]
    end

    D --> K[Domain Pack<br/>行业包/规则/SOP]
    K --> G
    K --> I
    K --> L
