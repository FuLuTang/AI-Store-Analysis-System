...
flowchart TD
    ...
    C --> D[API: 创建 run 目录 / 写 session.json / 追加 runs.json 列表]
    ...
    subgraph P2[Agent]
        ...
        H --> I[写 指标.json]
        I --> J[写 output/summary.md / output/summary_short.json]
    end

        J --> K[Pipeline 回收产物]
        K --> L[组装 output/result.json]
        L --> M[写 latest_report.md / session.json]
        M --> U[API: 更新 runs.json 状态为完成或失败]
        U --> N[XiaoTangPush 推送商搏 ERP]

    N --> O[logs.json / SSE]
    M --> O
    D --> O
    Q[POST /api/stop] --> R[force_stop + cancel task + aborted / 更新 runs.json 状态为终止]
    R --> O
...
- 完整报告：`output/summary.md`（markdown 格式），包含指标计算结果、异常数据、趋势分析、关联分析、建议等
- 精简报告：`output/summary_short.json`，包含指标计算结果、异常数据、趋势分析、关联分析、建议等，但格式为 JSON
...
4. 指标结果写入 `指标.json`
5. 报告写入 `output/summary.md` 和 `output/summary_short.json`
