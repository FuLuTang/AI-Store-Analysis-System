...
### 2.4 `replace_text`
* **功能描述**：在工作区文本文件中执行严格的唯一匹配替换。只有 `old_text` 在目标文件中恰好出现 1 次时才允许替换；匹配数为 `0` 或大于等于 `2` 都会报错，避免误改多处内容。
* **参数列表**：

| 参数名 | 类型 | 是否必填 | 默认值 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `path` | string | 是 | - | 相对工作区的目标文件路径。 |
| `old_text` | string | 是 | - | 需要被替换的原始文本，要求在文件中唯一匹配。 |
...
### 2.5 `copy_file`
* **功能描述**：复制工作区内的单个文件到新路径。目标父目录若不存在会自动创建；若目标文件已存在则覆盖。
* **参数列表**：

| 参数名 | 类型 | 是否必填 | 默认值 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `source_path` | string | 是 | - | 源文件在工作区内的相对路径。 |
| `destination_path` | string | 是 | - | 目标文件在工作区内的相对路径。 |
...
* **返回格式 (JSON 对象)**：
  ```json
  {
    ...
    "destination_path": "archive/report.md",
    "size": 2048,
    "size_human": "2.0KB"
  }
  ```
...
为了规范 Agent 运行逻辑并支撑可视化进度条，系统提供了五个辅助工作流状态的工具：
...
| **`finish_task`** | `success` (boolean, 必填), `text` (string, 必填) | 成功结束或报错终止当前任务。`success=true` 时系统会校验所有计划步骤是否都已经通过 `check_plan`。 |
...
| 参数名 | 类型 | 是否必填 | 默认值 | 描述 |
| :--- | :--- | :--- | :--- | :--- |
| `success` | boolean | 是 | - | `true` 表示成功结束任务，`false` 表示报错终止。 |
| `text` | string | 是 | - | 成功时填写最终总结；失败时填写具体错误原因。 |
