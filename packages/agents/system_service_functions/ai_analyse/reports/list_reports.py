...
def run(ws, params: dict, llm_preset: dict) -> str:
    ...
    try:
        with httpx.Client(timeout=httpx.Timeout(30.0)) as client:
            response = client.get(f"{api_base_url(params)}/api/reports", headers=headers)
    except Exception as e:
        return f"错误：请求 /api/reports 失败: {str(e)}"

    if response.status_code >= 400:
        return f"错误：/api/reports 返回失败，HTTP {response.status_code}: {api_error_message(response)}"
