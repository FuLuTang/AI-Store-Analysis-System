...
PLAN_TEMPLATE = [
    {"title": "查看输入文件",
     ...
     "status": "pending",
     "checks": [{"type": "glob_exists", "pattern": "input/*"}],
     "errors": []},
    {"title": "展平并输出 parquet",
     "detail": "写 Python 递归展平嵌套数据为二维表，推荐用 pandas.to_parquet 输出到 tables/。检查: pd.read_parquet 读回验证 row_count > 0。",
     "status": "pending",
     "checks": [
         {"type": "glob_exists", "pattern": "tables/*.parquet"},
         {"type": "parquet_non_empty", "pattern": "tables/*.parquet"},
     ],
     "errors": []},
    {"title": "入库，用 duckdb_register_parquet 注册表",
     ...
     "status": "pending",
     "checks": [{"type": "duckdb_query_ok", "sql": "SELECT table_name FROM information_schema.tables WHERE table_schema='main'"}],
     "errors": []},
    {"title": "画像，推荐用 profile_table 或 duckdb_query 探索字段",
     ...
     "status": "pending",
     "checks": [{"type": "duckdb_query_ok", "sql": "SELECT COUNT(*) FROM information_schema.columns LIMIT 1"}],
     "errors": []},
    ...
    {"title": "输出 AgentResult",
     ...
     "checks": [
         ...
         {"type": "validate_result", "path": "output/result.json"},
     ],
     "errors": []},
    {"title": "清理大文件",
